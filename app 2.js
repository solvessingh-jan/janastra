...
document.querySelectorAll('.tab').forEach(tab => {
  tab.addEventListener('click', () => {
    ...
    if (outputText) {
      outputText.textContent = `Selected: ${tabName.toUpperCase()} tab\nReady to receive reports...`;
...
async function submitReport(tableName, data, buttonElement) {
  ...
  if (outputText) outputText.textContent = 'Submitting your report...';
  ...
  try {
    ...
    if (outputText) {
      outputText.textContent = `✅ Success! Your ${tableName.replace('_', ' ')} report has been submitted.\n\nID: ${result.id}\nTime: ${new Date().toLocaleString('en-IN', { timeZone: 'Asia/Kolkata' })}\n\nThank you for helping your community!`;
    }
...
async function checkWaterStatus() {
  ...
  output.className = 'output loading';
  if (outputText) outputText.textContent = 'Checking water status...';
  ...
  try {
    ...
    if (!data || data.length === 0) {
      if (outputText) outputText.textContent = `No recent reports found for "${area}".\nBe the first to report!`;
    ...
    if (outputText) {
      outputText.textContent = `Water Status in "${area}":\n\n${statusText}\n\nLast updated: ${new Date(data[0].reported_at).toLocaleString('en-IN')}`;
    }
    output.className = 'output success';
  } catch (error) {
    if (outputText) outputText.textContent = `❌ Error: ${error.message}`;
    output.className = 'output error';
  }
}
...
function viewCivicMap() {
  ...
  window.open(mapsUrl, '_blank');
  showMessage('🗺️ Opening civic issues map in new tab...', 'success');
}
...
async function viewAreaIssues() {
  ...
  output.className = 'output loading';
  if (outputText) outputText.textContent = 'Fetching area issues...';
  ...
  try {
    ...
    if (!data || data.length === 0) {
      if (outputText) outputText.textContent = `No civic issues reported for "${area}".\nBe the first to report!`;
      output.className = 'output';
      return;
    }
    ...
    const categoryText = Object.entries(categoryCounts)
      .map(([category, count]) => `${category}: ${count} reports`)
      .join('\n');
    
    if (outputText) {
      outputText.textContent = `Civic Issues in "${area}":\n\n${categoryText}\n\nTotal: ${data.length} reports\nLast updated: ${new Date(data[0].reported_at).toLocaleString('en-IN')}`;
    }
    output.className = 'output success';
  } catch (error) {
    if (outputText) outputText.textContent = `❌ Error: ${error.message}`;
...
async function getTrafficReport() {
  ...
  output.className = 'output loading';
  if (outputText) outputText.textContent = 'Fetching traffic reports...';
  ...
  try {
    ...
    if (!data || data.length === 0) {
      if (outputText) {
        outputText.textContent = `No recent traffic reports for "${area}".\nBe the first to report current conditions!`;
      }
      output.className = 'output';
      return;
    }
    ...
    if (outputText) {
      outputText.textContent = `Traffic Reports for "${area}":\n\n${recentReports}\n\n💡 Tip: Check live map for real-time traffic colors`;
    }
    output.className = 'output success';
  } catch (error) {
    if (outputText) outputText.textContent = `❌ Error: ${error.message}`;
...
function showScamTab(tab) {
  const clickedButton = event.target.closest('.scam-tab');
...
async function checkPhoneNumber() {
  ...
  try {
    ...
    const output = document.getElementById('output');
    const outputText = document.querySelector('.output-text');
    if (output && outputText) outputText.textContent = '';
  } catch (error) {
...
async function loadClinics() {
  ...
  if (outputText) outputText.textContent = '🔍 Searching for clinics nearby...';
  output.className = 'output loading';
  ...
  if (!data || data.length === 0) {
    if (outputText) {
      outputText.textContent = `No clinics found in "${area}"${specialty ? ` with specialty: ${specialty}` : ''}.\n\nTry searching nearby areas or view map for more options.`;
    }
    output.className = 'output';
    return;
  }
  ...
  if (outputText) {
    outputText.textContent = `Found ${data.length} clinic(s) in "${area}":\n\n${clinicsList}\n\n💡 Click "View on Map" to see locations`;
  }
  output.className = 'output success';
}
